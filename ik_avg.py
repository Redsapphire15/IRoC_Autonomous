...
#lets try the reverse in this case.. we'll take average of last 10 values that came into the code
class IK_IROC():
    def __init__(self):
#        self.enc_data = [0,0,0]
#        self.d=[0,0,0,0] #joint lengths
        ...
        self.x_list, self.y_list, self.z_list, self.memory_list = [], [], [], []
    ...
    def angle_value(self):
        ...
        if(self.arm_goal_coming_from_tanish == False) or (self.listen_to_tanish == False):
            ...
            if(len(self.x_list) <=10):
                self.x_list.append(self.goal[0])
                self.y_list.append(self.goal[1])
                self.z_list.append(self.goal[2])
