...
#        self.d=[0,0,0,0] #joint lengths
    ...
    def feedback(self):
        ...
        if not cap.isOpened():
            print("Error trying to open the camera")
            exit()
        while True:
            ret, frame = cap.read()
            if not ret:
                print("Error trying to get the frame")
                break
            hsvframe = cv.cvtColor(frame, cv.COLOR_BGR2HSV)
            lower = np.array([0,200, 150])
            ...
            if bbox is not None:
                x1, y1, x2, y2 = bbox
                cv.rectangle(frame, (x1,y1), (x2,y2), (255, 255, 0), 5)
            else:
                self.listen_to_tanish = True
                self.tanish_count = 0
                self.goal_reached = False
                self.start_time = time.time()
                if abs(time.time() - self.start_time) <= 2:
                    msg=Int32MultiArray()
                    msg.data=[0,0,0,0,0,0]
                    msg.layout = MultiArrayLayout()
                    msg.layout.data_offset = 0
